...
SUMMERY_FILE = ".report_ai_summary"
...
def load_summery():
    if os.path.exists(".report_ai_summary"):
        with open(SUMMERY_FILE, "r", encoding="utf-8") as f:
            lines = f.read().splitlines()
            return lines
            
    return None

def save_summery(last_summery):
    with open(SUMMERY_FILE, "w", encoding="utf-8") as f:
        f.write(last_summery)
        return "last summery saved"
...
def add_to_session(session_target, message, command):
    ...
    print("Reading current last summery about report...")
    last_summery_of_report = load_summery()
            
    print("Sending data to Mistral AI for context-aware blending...")
    
    system_prompt = (
    "You are a specialized penetration testing report generator. "
    "You will be given a short summary of the report so far, a description of the tester's action, "
    "the command executed, and its raw output. "
    "Do two things:\n"
    "1. Write ONLY the new markdown section for this finding, in the same style/flow as the report so far. "
    "Do not repeat earlier content. Format command evidence in markdown code blocks.\n"
    "2. Write a short updated summary (2-4 sentences) covering everything found so far, including this new finding.\n\n"
    ...
    user_content = (
        ...
        f"--- SUMMARY SO FAR ---\n{last_summery_of_report if last_summery_of_report else 'No prior findings yet, this is the first entry'}\n\n"
        f"--- TESTER OBJECTIVE ---\n{message}\n\n"
        f"--- COMMAND EXECUTED ---\n{command}\n\n"
    ...
    updated_report = updated_report.split("###SUMMARY###")
    
    new_report_section = updated_report[0]
    new_report_section = new_report_section.strip()
    new_report_section = new_report_section.replace("###SECTION###", "")
    new_summery = updated_report[1]
    
    if new_report_section.strip().startswith("```markdown"):
        new_report_section = new_report_section.strip()[11:]
        
    elif new_report_section.strip().startswith("```md"):
        new_report_section = new_report_section.strip()[5:]
        
    if new_report_section.strip().endswith("```"):
        new_report_section = new_report_section.strip()[:-3]
        
    with open(filename, "a", encoding="utf-8") as f:
        f.write(new_report_section.strip() + "\n")
        
    print(f"Successfully updated {filename} with new findings.")
    
    save_summery(new_summery)
